...
class QwenVLLMQuery(GPT4Query):
    ...
    def prepare_images(self) -> List[Image.Image]:
        """Prepare images（few-shot template + test image ）"""
        images = []
    ...
    def generate_response_vllm(self, prompt: str, images: List[Image.Image]) -> str:
        # vLLM expects a single input format
...
def get_ans(response_text, options=None):
    try:
        # Try to match the <answer> tag
        ans_match = re.search(r'<answer>(.*?)</answer>', response_text)
        gpt_answer = ans_match.group(1).strip().lower()  # to lower
...
if __name__ == "__main__":
    ...
    parser.add_argument("--name", type=str, default="Qwen")
