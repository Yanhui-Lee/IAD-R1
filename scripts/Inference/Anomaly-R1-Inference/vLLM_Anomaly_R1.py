...
class QwenVLLMQuery(GPT4Query):
    def __init__(self, image_path, text_gt, llm, tokenizer, few_shot=[], visualization=False, args=None):
        super(QwenVLLMQuery, self).__init__(image_path, text_gt, few_shot, visualization)
        self.llm = llm
    ...
    def prepare_images(self) -> List[Image.Image]:
        ...
        for ref_image_path in self.few_shot:
            ...
            images.append(Image.open(ref_image_path))
        
        images.append(Image.open(self.image_path))
    ...
    def build_prompt(self, conversation) -> str:
        ...
        system_message = ('First output the thinking process in <think> </think> tags and then output the final answer letter in <answer> </answer> tags.')
...
def batch_process_with_vllm(llm, tokenizer, batch_data, args):
    ...
    for item in batch_data:
        ...
        questions, answers = qwen_query.parse_conversation(text_gt)
        if questions and answers:
            questions = questions[0:1]
            answers = answers[0:1]
            
            prompt = qwen_query.build_prompt(questions)
            images = qwen_query.prepare_images()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--name", type=str, default="anomaly-r1")
    ...
    for i in tqdm(range(0, len(batch_data), args.batch_size), desc="Processing batches", total=total_batches):
        ...
        for result in results:
            ...
            final_responses = result['final_response'] 
            ...
            for q, a, ga, qt, fr in zip(questions, answers, gpt_answers, questions_type, final_responses):
                answer_entry = {
                    "image": original_path,
                    "question": q,
                    "question_type": qt,
